...
├── scripts/
│   └── seed_test_db.sql     # seeds crypto_assets for the federated metrics test; all other live tests create their own uniquely-named fixtures
├── dummy_iceberg_cdc/
...
└── src/
    ...
    ├── pushdown.rs          # pure Expr -> SQL WHERE translation for pushdown (F3.1)
...
Public API: `struct PostgresTable`; `async fn try_new(conn_str, schema_name, table_name, schema) -> IglooResult<Self>`; `fn from_client(client, schema_name, table_name, schema)` for catalog registration sharing one connection; `with_filter_pushdown(bool)` to disable pushdown (differential testing); `rows_fetched()` telemetry; `impl TableProvider`. Private pure helpers: `quote_ident`/`quote_relation` (scans are schema-qualified so tables outside `search_path` resolve) and the SQL assembler `build_scan_sql`; the filter translation itself lives in `src/pushdown.rs`.
Key decisions:
...
- **Filter pushdown** (F3.1, `src/pushdown.rs`): `supports_filters_pushdown` reports `Inexact` for every filter `pushdown::translate_filter` can render as a PostgreSQL snippet, `Unsupported` otherwise; `scan` translates with the same single source of truth and emits the clauses as `WHERE ... AND ...`. Grammar: column-vs-literal comparisons (either operand order, operator flipped as needed) on bools, ints, finite floats and strings — with string operands restricted to `=`/`<>` (this branch's collation gate: PostgreSQL text ordering diverges from Arrow byte ordering in both directions, so a pushed string ordering could under-fetch, which the `Inexact` re-filter cannot repair; equality assumes deterministic collations) — plus `IS [NOT] NULL`, `[NOT] IN` literal lists, and `AND`. NULL literals, NaN/inf, dates/timestamps/decimals, casts, `OR`, LIKE, and column-vs-column are rejected. `Inexact` means DataFusion re-applies each pushed predicate above the scan; `pushed_limit` additionally drops a pushed `LIMIT` unless every filter was translated, keeping `WHERE`+`LIMIT` composition sound. A `filter_pushdown: false` toggle turns the whole feature off for differential testing.
...
Tests: unit tests cover `quote_ident`, `build_scan_sql` (incl. WHERE/LIMIT/COUNT paths) and `pushed_limit`; 4 nested live integration tests (gated on `IGLOO_TEST_POSTGRES_URI`) assert results through a DataFusion `SessionContext` incl. the re-filtered string-ordering case and the COUNT path. Translation itself is tested in `src/pushdown.rs` (27 unit tests) and end-to-end in `tests/pushdown.rs` (differential pushed-vs-unpushed, hostile literals); `try_new`'s failure paths remain untested.
...
| `postgres_table.rs` | 18 unit + 4 integration | `quote_ident`, `build_scan_sql` (WHERE/LIMIT/COUNT paths, qualification, quoting), `pushed_limit`; live integration (gated on `IGLOO_TEST_POSTGRES_URI`, seeded via `scripts/seed_test_db.sql`): pushed `=`, pushed `IS NULL`, string-ordering re-filtered above the scan, filtered COUNT. |
| `pushdown.rs` | 27 | translation grammar: all operators, operand flipping, IN lists, hostile literals (quote/NUL injection), NaN/inf rejection, string-ordering rejection (collation gate), unsupported constructs. |
| `tests/pushdown.rs` | (env-gated) | differential pushed-vs-unpushed execution against live PostgreSQL asserting identical results plus reduced rows fetched. |
...
| 4 | ~~No filter pushdown~~ **Resolved on `main` (F3.1)** with this branch adding the string-ordering collation gate. Residual: unsupported constructs (`OR`, LIKE, dates/timestamps/decimals, string ordering) still fetch the full table and filter locally | Low (residual) | `src/pushdown.rs`; gate at `translate_comparison`/`is_string_literal` |
...
| 17 | ~~Declared MSRV `rust-version = "1.80.1"` is not satisfiable with the committed lockfile~~ **Resolved**: `rust-version` raised to **1.88**, the maximum `rust_version` across the locked dependency graph (`ar_archive_writer 0.5.2` requires 1.88; a broad band of crypto/hash crates requires ≥ 1.85). Verified by building and testing with an isolated 1.88 toolchain (`cargo check --all-targets --locked` + `cargo test --locked`, both green), and now enforced on every PR by the CI `msrv` job, which reads the version from `Cargo.toml` so there is one source of truth. | — | `Cargo.toml:5`; `.github/workflows/rust.yml` (`msrv` job) |
